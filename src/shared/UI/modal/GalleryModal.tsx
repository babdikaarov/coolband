...
const GalleryModal: React.FC<GalleryModalProps> = ({ index, imageList, showOverlay, setIndex, setShowOverlay }) => {
  if (!showOverlay) return null;

  return (
    <>
      <div className="overlay" onClick={() => setShowOverlay(!showOverlay)}></div>
      <div className={!showOverlay ? "gallery-overlay modal-active" : "gallery-overlay"}>
        <button className="gallery-overlay-left" onClick={() => setIndex((prev) => prev - 1)}>
        ...
        <button className="gallery-overlay-right" onClick={() => setIndex((prev) => prev + 1)}>
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" fill="none">
